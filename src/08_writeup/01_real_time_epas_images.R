...
# Example 1: Regular Completion -------------------------------------------

# gameId: 2018123011
# playId: 1573

pbp_data_completion = pbp_data_clean %>%
  inner_join(plays %>%
               filter(passResult == 'C',
                      epa < 1,
                      epa > .5) %>%
...
# clipping the play
play1 = pbp_data_clean %>%
  filter(gameId == 2018123011,
         playId == 1573)
...
pass_forward1 = 
  ...
  facet_wrap(~event) +
  labs(title = paste0("EPA Prediction = ", gsub("-", "\u2013", toString(replace_na(round(play1_desc$epa_pass_attempt, 3), "N/A")))),
       subtitle = paste0("C% = ", gsub("-", "\u2013", toString(replace_na(scales::percent(round(play1_desc$c_prob_pass_attempt, 4), .1), "N/A"))),
       ", INT% = ", gsub("-", "\u2013", toString(replace_na(scales::percent(round(play1_desc$in_prob_pass_attempt, 4), .1), "N/A")))
))
...
pass_caught1 = 
  ...
  labs(title = paste0("EPA Prediction = ", gsub("-", "\u2013", toString(replace_na(round(play1_desc$epa_throw, 3), "N/A")))),
       subtitle = paste0("Expected YAC = ", gsub("-", "\u2013", toString(replace_na(round(play1_desc$eyac, 3), "N/A"))))
       )
...
pass_tackle1 = 
  ...
  theme(axis.line=element_blank(),
        ...
        plot.subtitle = element_text(size=10)) +
  facet_wrap(~event) +
  labs(title = paste0("EPA (nflWAR) = ", gsub("-", "\u2013", toString(replace_na(round(play1_desc$epa, 3), "N/A")))),
       subtitle = ""
  )
...
gridExtra::grid.arrange(pass_forward1, 
                        pass_arrived1,
                        pass_caught1,
                        pass_tackle1,
                        ncol = 4,
...
g1 <- gridExtra::arrangeGrob(pass_forward1, 
                             pass_arrived1,
                             pass_caught1,
                             pass_tackle1,
                             ncol = 4,
                             top = paste0(game.info$visitorTeamAbbr, " @ ", game.info$homeTeamAbbr,
                                          ...
                                          paste(strwrap(paste("Play Description:", toString(play1_desc$playDescription))), collapse="\n")))
ggsave(plot = g1, 
       filename = "~/Desktop/CoverageNet/src/08_writeup/images/ScoredEPAsEx1.png",
       height = 4,
       width = 10)

# Example 2: Bad Tackle -------------------------------------------------

# gameId: 2018123008
# playId: 1687

pbp_data_good_tackle = pbp_data_clean %>%
  inner_join(plays %>%
               filter(passResult == 'C',
                      down == 3,
                      playResult < 20) %>%
...
play2 = pbp_data_clean %>%
  filter(gameId == 2018123008,
         playId == 1687)
...
pass_arrived2 = 
  ...
  theme(axis.line=element_blank(),
        ...
        plot.subtitle = element_text(size=10)) +
  facet_wrap(~event) +
  labs(title = paste0("EPA Prediction = ", gsub("-", "\u2013", toString(replace_na(round(play2_desc$epa_pass_arrived, 3), "N/A")))),
       subtitle = paste0("C% = ", gsub("-", "\u2013", toString(replace_na(scales::percent(round(play2_desc$c_prob_pass_arrived, 4), accuracy = 0.1), "N/A"))),
                         ", INT% = ", gsub("-", "\u2013", toString(replace_na(scales::percent(round(play2_desc$in_prob_pass_arrived, 4), accuracy = 0.1), "N/A")))
       ))
...
g2 <- gridExtra::arrangeGrob(pass_forward2, 
                             pass_arrived2,
                             pass_caught2,
                             pass_tackle2,
                             ncol = 4,
                             top = paste0(game.info$visitorTeamAbbr, " @ ", game.info$homeTeamAbbr,
                                          ...
                                          paste(strwrap(paste("Play Description:", toString(play2_desc$playDescription))), collapse="\n")))
ggsave(plot = g2, 
       filename = "~/Desktop/CoverageNet/src/08_writeup/images/ScoredEPAsEx2.png",
       height = 4,
       width = 10)
...
pbp_data_interception = pbp_data_clean %>%
  inner_join(plays %>%
               filter(passResult == 'IN')) %>%
  inner_join(pass_arrived_epa_data %>%
               filter(epa_pass_arrived < 0) %>%
               distinct(gameId, playId))

play3 = pbp_data_clean %>%
  filter(gameId == 2018123011,
         playId == 4261)
...
pass_caught3 = 
  ...
  theme(axis.line=element_blank(),
        ...
        plot.subtitle = element_text(size=10)) +
  facet_wrap(~event) +
  labs(title = paste0("EPA Prediction = ", gsub("-", "\u2013", toString(replace_na(round(play3_desc$epa_throw, 3), "N/A")))),
       subtitle = paste0("Expected YAINT = ", gsub("-", "\u2013", toString(replace_na(round(play3_desc$eyaint, 3), "N/A"))))
  )
...
pass_tackle3 = 
  ...
  geom_segment(data = play3_clipped %>%
               ...
               arrow = arrow(length = unit(.25,"cm"))) +
  geom_point(data = play3_clipped %>%
               filter(event == "out_of_bounds"), aes(x = (xmax-y), y = x + 10, shape = team,
                                              fill = team, group = nflId, size = team, colour = team), alpha = 0.9) +
  geom_point(data = play3_clipped %>%
               inner_join(targeted_receiver %>% rename(nflId = targetNflId)) %>%
               filter(event == "out_of_bounds"), aes(x = (xmax-y), y = x + 10),
             size = 4, color = "black", shape = 21, fill = "#e31837", alpha = 0.9) +
  geom_text(data = play3_clipped %>%
              filter(event == "out_of_bounds") %>%
              filter(!is.na(nflId)), aes(x = (xmax-y), y = x + 10, label = jerseyNumber, group = nflId), colour = "white",
  ...
  theme(axis.line=element_blank(),
        ...
        plot.subtitle = element_text(size=10)) +
  facet_wrap(~event) +
  labs(title = paste0("EPA (nflWAR) = ", gsub("-", "\u2013", toString(replace_na(round(play3_desc$epa, 3), "N/A")))),
       subtitle = ""
  )
...
g3 <- gridExtra::arrangeGrob(pass_forward3, 
                             ...
                             top = paste0(game.info$visitorTeamAbbr, " @ ", game.info$homeTeamAbbr,
                                          ...
                                          paste(strwrap(paste("Play Description:", toString(play3_desc$playDescription))), collapse="\n")))
ggsave(plot = g3, 
       filename = "~/Desktop/CoverageNet/src/08_writeup/images/ScoredEPAsEx3.png",
       height = 5.5,
       width = 10)


# Animate Tracking EPAs example -------------------------------------------
...
animate.play = 
  ...
  geom_point(data = example.play %>%
               inner_join(targeted_receiver %>% rename(nflId = targetNflId)), aes(x = (xmax-y), y = x + 10),
...
library(gifski)
anim_save("~/Desktop/CoverageNet/src/08_writeup/images/TrackingEPAsEx.gif",
          new_gif,
          fps = 5,
