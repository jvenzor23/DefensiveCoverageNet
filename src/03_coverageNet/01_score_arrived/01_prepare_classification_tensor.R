# This code prepares the tensor for C/I/INT predictions
...
# checking that distance between ball and targeted receiver is reasonable
plays_remove5 = pass_attempts %>%
  filter(IsReceiver|(is.na(nflId))) %>%
  group_by(gameId, playId) %>%
  summarize(football_receiver_diff = sqrt((x[1] - x[2])^2 + (y[1] - y[2])^2),
            receiver = nflId[!is.na(nflId)]) %>%
  filter(football_receiver_diff > 7.5)
...
write.csv(tot_data5, "pass_arrived_training_tensor.csv", row.names = FALSE)
write.csv(target2, "pass_arrived_training_target.csv", row.names = FALSE)
...
pass_attempts = read.csv("~/Desktop/CoverageNet/src/03_coverageNet/01_score_arrived/outputs/pass_attempts_with_fumbles.csv")
...
setwd("~/Desktop/CoverageNet/src/03_coverageNet/01_score_arrived/outputs/")
write.csv(tot_data4, "pass_arrived_score_tensor.csv", row.names = FALSE)
