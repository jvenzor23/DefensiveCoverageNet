# This code prepares the tensor for INT yards prediction
...
target = tot_data4 %>%
  ...
  mutate(offensePlayResult_adj = case_when(offensePlayResult < -50 ~ as.numeric(-50),
...
write.csv(tot_data5, "interception_training_tensor.csv", row.names = FALSE)
write.csv(target2, "interception_training_target.csv", row.names = FALSE)
