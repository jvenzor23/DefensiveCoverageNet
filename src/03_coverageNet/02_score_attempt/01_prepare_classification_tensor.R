# This code prepares the tensor for C/I/INT prediction
...
pass_attempts = read.csv("~/Desktop/CoverageNet/src/03_coverageNet/02_score_attempt/outputs/pass_attempts.csv")
...
target = tot_data4 %>%
  ...
  inner_join(pass_attempts_tot %>%
               distinct(gameId, playId, passResult)) %>%
  distinct() %>%
  arrange(group, gameId, playId2) %>%
  mutate(passResultDisc = case_when(passResult == "IN" ~ 0,
                                    passResult == "I" ~ 1,
                                    passResult == "C" ~ 2,
                                    TRUE ~ as.numeric(NA)))
...
setwd("~/Desktop/CoverageNet/src/03_coverageNet/02_score_attempt/outputs/")
write.csv(tot_data5, "pass_attempt_training_tensor.csv", row.names = FALSE)
write.csv(target2, "pass_attempt_training_target.csv", row.names = FALSE)
...
ggplot(colour = "black")  + 
  geom_point(data = plt_comp2 %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_comp2 %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_comp2 %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_comp2 %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_comp2$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
ggplot(colour = "black")  + 
  geom_point(data = plt_comp3 %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_comp3 %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_comp3 %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_comp3 %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_comp3$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
ggplot(colour = "black")  + 
  geom_point(data = plt_incomp %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_incomp %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_incomp %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_incomp %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_incomp$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
ggplot(colour = "black")  + 
  geom_point(data = plt_incomp2 %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_incomp2 %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_incomp2 %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_incomp2 %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_incomp2$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
ggplot(colour = "black")  + 
  geom_point(data = plt_int %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_int %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_int %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_int %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_int$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
ggplot(colour = "black")  + 
  geom_point(data = plt_int2 %>% filter(!is.na(nflId)), 
             aes(x, y, fill = grouping), size = 4, pch = 21) + 
  geom_point(data = plt_int2 %>% filter(is.na(nflId)), 
             aes(x, y), fill = 'brown', size = 3, pch = 21) + 
  geom_segment(data = plt_int2 %>% filter(!is.na(nflId)),
               aes(x = x, y = y, xend = x_end,
                   yend = y_end, colour = grouping), 
               arrow = arrow(length = unit(.5,"cm"))) + 
  scale_colour_brewer(palette = "Set2")+ 
  scale_fill_brewer(palette = "Set2")+ 
  geom_vline(data = plt_int2 %>% filter(!is.na(nflId)),
             aes(xintercept = YardsFromOwnGoal), 
             colour = "black", lty = 2) + 
  scale_x_continuous(breaks = c(7:10)*10) + 
  labs(x = "Distance from offensive team's own end zone", 
       y = "Y coordinate",
       title = unique(plt_int2$gameId)) + 
  theme_bw(14) + 
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major.y =element_blank(),
        legend.title = element_blank()) +
...
pass_attempts = read.csv("~/Desktop/CoverageNet/src/03_coverageNet/02_score_attempt/outputs/pass_attempts_with_fumbles.csv")
...
setwd("~/Desktop/CoverageNet/src/03_coverageNet/02_score_attempt/outputs/")
write.csv(tot_data4, "pass_attempt_score_tensor.csv", row.names = FALSE)


plt_int = pass_attempts %>%
  inner_join(pass_attempts %>%
               filter(gameId == 2018110500,
                      playId == 3845) %>%
               select(gameId, playId) %>%
               distinct() %>%
               filter(row_number() == 1)) %>%
