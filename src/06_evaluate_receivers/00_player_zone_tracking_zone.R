...
time_to_hold_dist = epa_tracking_total_penalties_removed %>%
  anti_join(plays %>%
              filter(passResult == 'S') %>%
...
time_to_hold_dist2$fitted_perc = sn_fitted_vals/10
...
max_zone_coverage_tracking = epa_tracking_total_penalties_removed %>%
  ...
  ungroup() %>%
  group_by(targetNflId, time_after_entering_zone) %>%
  summarise(count = n(),
            avg_max_epa = mean(epa_pass_attempt_max)) %>%
  arrange(targetNflId, time_after_entering_zone) %>%
  group_by(targetNflId) %>%
  mutate(max_time_after_entering_zone = max(time_after_entering_zone)) %>%
  ungroup() %>%
  filter(count >= 5)

max_zone_coverage_tracking_filled = max_zone_coverage_tracking %>%
  ungroup() %>%
  complete(targetNflId, time_after_entering_zone)

max_zone_coverage_tracking_filled = max_zone_coverage_tracking_filled %>%
  left_join(epa_tracking_total_penalties_removed %>%
              ...
              dplyr::select(-max_time_after_entering_zone)) %>%
...
max_zone_coverage_tracking_filled$count[is.na(max_zone_coverage_tracking_filled$count)] = 1

max_zone_coverage_tracking_filled = max_zone_coverage_tracking_filled %>%
  fill(avg_max_epa) %>%
  fill(max_time_after_entering_zone)
...
# return a list
models <- plyr::dlply(max_zone_coverage_tracking_filled, "targetNflId", function(df) 
...
fit_by_player2 = fit_by_player %>%
  pivot_longer(cols = setdiff(names(fit_by_player), "targetNflId"),
               names_to = "time_after_entering_zone_placeholder",
               values_to = "avg_max_epa_pred") %>%
  group_by(targetNflId) %>%
  mutate(time_after_entering_zone = row_number()*.1 - .1) %>%
  dplyr::select(-time_after_entering_zone_placeholder)

max_zone_coverage_tracking_filled2 = max_zone_coverage_tracking_filled %>%
  mutate(time_after_entering_zone = as.factor(time_after_entering_zone)) %>%
  inner_join(fit_by_player2 %>%
               mutate(time_after_entering_zone = as.factor(time_after_entering_zone)))

max_zone_coverage_tracking_filled2$eps_tracking_avg = predict(epa_to_eps_model,
                                                              max_zone_coverage_tracking_filled2 %>%
                                                                   rename(max_epa_disc = avg_max_epa_pred))

max_zone_coverage_tracking_filled2 = max_zone_coverage_tracking_filled2 %>%
...
write.csv(max_zone_coverage_tracking_filled4,
          "~/Desktop/CoverageNet/src/06_evaluate_receivers/outputs/receiver_zone_tracking_eps.csv",
          row.names = FALSE)
