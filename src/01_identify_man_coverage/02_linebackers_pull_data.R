...
for(file in files){
  ...
  pbp_data3_corners = pbp_data3 %>%
    filter(position %in% c('LB', 'OLB', 'ILB', 'MLB'))
  ...
  write.csv(pass_attempt_total2,
            "~/Desktop/CoverageNet/src/01_identify_man_coverage/outputs/linebackers_pass_attempts_man_zone_gmm_features.csv",
            row.names = FALSE)
  
  write.csv(sack_total2,
            "~/Desktop/CoverageNet/src/01_identify_man_coverage/outputs/linebackers_sacks_man_zone_gmm_features.csv",
            row.names = FALSE)
