...
wr_db_zone_matchups_tot = read.csv("~/Desktop/CoverageNet/src/01_identify_man_coverage/outputs/zone_defense_off_coverage_assignments.csv")
...
max_zone_coverage_tracking_filled = max_zone_coverage_tracking %>%
  ungroup() %>%
  complete(nflId, time_after_entering_zone)
...
fit_by_player2 = fit_by_player %>%
  pivot_longer(cols = setdiff(names(fit_by_player), "nflId"),
...
player_probs_vs_time = max_zone_coverage_tracking_filled %>%
  mutate(time_after_entering_zone = as.factor(time_after_entering_zone)) %>%
  inner_join(fit_by_player2 %>%
               mutate(time_after_entering_zone = as.factor(time_after_entering_zone))) %>%
  inner_join(players %>%
